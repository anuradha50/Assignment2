...
app.config(function($stateProvider){
    ...
    $stateProvider
        ...
        .state('app.dashboard',{
            url: '/app',
            views: {
                "viewA": { template: "<p> You already logged in. Hello world from a</p>"}
                "viewB": {template: "<p> you already logged in. Hello world from b</p>"}
            }
            // child state of 'app'
            // requireLogin === true
        })
...
app.config(function ($httpProvider) {
    $hhtpProvider.interceptors.push(function ($timeout, $q, $injector){
        var loginModal, $http, $state;
        // this trick must be done so that we don't receive
        //'Uncaught Error: [$injector:cdep] Circular dependency found'
        $timeout(function () {
            loginModal = $injector.get('loginModal');
            $http = $injector.get('$state');
...
app.service('loginModal', function (ngDialog, $rootScope) {
    ...
    return function() {
        var instance = ngDialog.openConfirm({
            template:'<div>\<form ng-submit="LoginModelCtrl.submit(_email, _password)'>\
